...
Installed as the ``sql-connect`` console script; also runnable via
``python scripts/connect.py`` (thin wrapper) or ``python -m sql_connection.cli``.

Examples
--------
# SQLite
sql-connect sqlite --path examples/toys_and_models.sqlite

# PostgreSQL
sql-connect postgres --host localhost --port 5432 \
...
def _run(engine: str, args: argparse.Namespace) -> int:
    if getattr(args, "verbose", False):
        logging.getLogger("sql_connection").setLevel(logging.DEBUG)
